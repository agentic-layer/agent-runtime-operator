...
spec:
  ...
  tools:
    ...
    # Direct URL to external MCP server (no ToolServer resource needed)
    - name: web_fetcher
      upstream:
        external:
          url: "https://mcp.example.com/web-fetch"

    # Optional: Propagate specific headers to the MCP server for authentication
    # Headers from incoming A2A requests are passed to the tool server
  ...
  subAgents:
    - name: summarizer_agent
      ...
      interactionType: "tool_call"  # Optional: treat subagent as tool call (default) rather than "transfer"ing the session to the subagent
...
==== Sub-Agent Interaction Types

When configuring sub-agents, you can control how your agent interacts with them using the optional `interactionType` field:

* *`tool_call` (default)*: The sub-agent is invoked as a regular tool call. The parent agent remains in control and receives the sub-agent's response as tool output.
* *`transfer`*: Uses the `transfer_to_agent` mechanism to hand off control to the sub-agent. The sub-agent takes over the conversation and can interact directly with the user.

[source,yaml]
----
subAgents:
  # Tool call: agent calls sub-agent and processes response
  - name: summarizer_agent
    url: "https://agents.example.com/summarizer-agent.json"
    interactionType: "tool_call"  # Default behavior

  # Transfer: hand off conversation to specialist
  - name: specialist_agent
    url: "https://agents.example.com/specialist-agent.json"
    interactionType: "transfer"
----

==== Tool Header Propagation

The `propagatedHeaders` field allows you to pass specific HTTP headers from incoming A2A requests to MCP tool servers. This enables authentication and authorization scenarios where tools need to access external APIs on behalf of users.
...
tools:
  - name: github_api
    ...
    propagatedHeaders:
      - Authorization      # Pass the Authorization header
      - X-GitHub-Token     # Pass custom headers

  - name: public_tool
    upstream:
      external:
        url: "https://public-mcp.example.com/mcp"
    # No propagatedHeaders - no headers are passed
----
...
* *Default behavior*: When `propagatedHeaders` is not specified or is empty, no headers are passed

*Example usage:* When a user makes an A2A request with an `Authorization` header, only the MCP servers configured with `propagatedHeaders: [Authorization]` will receive that header.

NOTE: Header propagation is only supported for MCP tool servers, not for sub-agents.
