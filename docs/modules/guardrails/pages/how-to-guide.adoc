...
Today, guardrails are implemented by the xref:ai-gateway-litellm-operator::how-to-guides/install.adoc[LiteLLM Gateway Operator] (Presidio-based PII guardrails on an `AiGateway`) and the xref:tool-gateway-agentgateway::how-to-guides/install.adoc[Tool Gateway Agentgateway Operator] (Presidio-based PII guardrails on a `ToolGateway`).
