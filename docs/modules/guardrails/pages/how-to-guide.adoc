...
|Yes
|`presidio-api`
|Implemented by the xref:tool-gateway-agentgateway:operator:how-to-guide.adoc[Tool Gateway Agentgateway Operator]. The operator deploys a per-Guard `guardrail-adapter` instance and wires it into the gateway's `AgentgatewayPolicy` via `ext_proc`. Today, only one Guard per `ToolGateway` is supported.
|===
