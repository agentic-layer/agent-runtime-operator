...
spec:
  ...
  protocols:
    - type: A2A
      port: 8000
`, parentAgentName, webhookv1alpha1.DefaultTemplateImageAdk, subAgentName)
			...
			cmd = exec.Command("kubectl", "delete", "agent", parentAgentName, "-n", testNamespace)
			_, _ = utils.Run(cmd)
			cmd = exec.Command("kubectl", "delete", "agent", subAgentName, "-n", testNamespace)
			_, _ = utils.Run(cmd)
		})
