...
var _ = Describe("Agent Config", func() {
	Describe("buildTemplateEnvironmentVars", func() {
			...
			agent := &runtimev1alpha1.Agent{
				...
				Spec: runtimev1alpha1.AgentSpec{
					...
					Tools: []runtimev1alpha1.AgentTool{
						{Name: "tool1", Upstream: runtimev1alpha1.AgentToolUpstream{ToolRouteRef: &corev1.ObjectReference{Name: "tool-server-1"}}},
						{Name: "tool2", Upstream: runtimev1alpha1.AgentToolUpstream{ToolRouteRef: &corev1.ObjectReference{Name: "tool-server-2"}}},
					},
				},
			}
		...
		It("should handle JSON marshaling of tools", func() {
			agent := &runtimev1alpha1.Agent{
				...
				Spec: runtimev1alpha1.AgentSpec{
					Tools: []runtimev1alpha1.AgentTool{
						{Name: "test-tool", Upstream: runtimev1alpha1.AgentToolUpstream{ToolRouteRef: &corev1.ObjectReference{Name: "tool-server-1"}}},
					},
				},
			}
		...
		It("should include propagate_headers in AGENT_TOOLS when specified", func() {
			agent := &runtimev1alpha1.Agent{
				...
				Spec: runtimev1alpha1.AgentSpec{
					Tools: []runtimev1alpha1.AgentTool{
						{Name: "auth-tool", Upstream: runtimev1alpha1.AgentToolUpstream{ToolRouteRef: &corev1.ObjectReference{Name: "tool-server-1"}}},
					},
				},
			}
		...
		It("should not include propagate_headers in AGENT_TOOLS when empty", func() {
			agent := &runtimev1alpha1.Agent{
				...
				Spec: runtimev1alpha1.AgentSpec{
					Tools: []runtimev1alpha1.AgentTool{
						{Name: "public-tool", Upstream: runtimev1alpha1.AgentToolUpstream{ToolRouteRef: &corev1.ObjectReference{Name: "tool-server-1"}}},
					},
				},
			}
	...
	Context("when aiGatewayUrl is provided", func() {
		...
		It("should allow user to override OTEL_SEMCONV_STABILITY_OPT_IN", func() {
			agent := &runtimev1alpha1.Agent{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "otel-override-agent",
					Namespace: "default",
				},
				Spec: runtimev1alpha1.AgentSpec{},
			}
