...
var _ = Describe("Agent Resources", func() {
		...
		It("should apply default resource requests and limits when not specified", func() {
			agent := &runtimev1alpha1.Agent{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-default-resources-agent",
					Namespace: "default",
				},
			...
			Expect(k8sClient.Get(ctx, types.NamespacedName{Name: "test-default-resources-agent", Namespace: "default"}, deployment)).To(Succeed())
	...
	Describe("Custom Resource Configuration", func() {
		It("should use custom resource requests and limits when specified", func() {
			...
			agent := &runtimev1alpha1.Agent{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-custom-resources-agent",
					Namespace: "default",
				},
				Spec: runtimev1alpha1.AgentSpec{
					Framework: "google-adk",
			...
			deployment := &appsv1.Deployment{}
			Expect(k8sClient.Get(ctx, types.NamespacedName{Name: "test-custom-resources-agent", Namespace: "default"}, deployment)).To(Succeed())
			Expect(deployment.Spec.Template.Spec.Containers).To(HaveLen(1))
		...
		It("should handle partial resource configuration", func() {
			...
			agent := &runtimev1alpha1.Agent{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-partial-resources-agent",
					Namespace: "default",
				},
				Spec: runtimev1alpha1.AgentSpec{
					Framework: "google-adk",
			...
			deployment := &appsv1.Deployment{}
			Expect(k8sClient.Get(ctx, types.NamespacedName{Name: "test-partial-resources-agent", Namespace: "default"}, deployment)).To(Succeed())
			Expect(deployment.Spec.Template.Spec.Containers).To(HaveLen(1))
