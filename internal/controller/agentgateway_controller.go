...
func (r *AgentGatewayReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if agentGateway.Status.Conditions == nil {
		agentGateway.Status.Conditions = []metav1.Condition{}
	}

	// Initialize error classifier if not set
	if r.ErrorClassifier == nil {
		r.ErrorClassifier = NewErrorClassifier()
	}
	...
	return r.updateStatus(ctx, &agentGateway, ctrl.Result{}, nil)
}
