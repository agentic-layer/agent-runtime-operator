...
// For cluster agents (no URL): looks up the Agent resource and uses its status.url
// If no namespace is specified, defaults to the parent agent's namespace
func (r *AgentReconciler) resolveSubAgentUrl(ctx context.Context, subAgent runtimev1alpha1.SubAgent, parentNamespace string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("failed to resolve cluster agent %s/%s: %w", namespace, subAgent.Name, err)
	}

	// Use the URL from the agent's status (populated by the controller)
	if referencedAgent.Status.Url == "" {
		return "", fmt.Errorf("cluster Agent %s/%s has no URL in its Status field (may not be ready or have A2A protocol)", namespace, subAgent.Name)
	}
